...
function dataURIPayload(string: string | number | boolean) {
...
function shortNames(string: string) {
...
function miniSVG(svg: SVGSVGElement) {
  // Convert svg to serialize string
  ...
  const body = shortNames(collapseWhitespace(svgString)).replace(
    REGEX.quotes,
    "'"
  );
...
miniSVG.toSrcset = function toSrcset(svgString: any) {
  return miniSVG(svgString).replace(/ /g, '%20');
};
